...
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=10       # 12 CPUs/task
#SBATCH --mem=128G
...
NOISE=${NOISE:-0.02}
HIDDEN_DIM=${HIDDEN_DIM:12}          # <-- new

# Create a unique job tag based on hyperparameters
JOB_TAG="e${EPOCHS}_b${BATCH_SIZE}_n${NEIGHBORS}_h${HIDDEN_DIM}_noise${NOISE}"

# Define output and log directories
OUTPUT_DIR="outputs/dim_${JOB_TAG}"
LOG_DIR="logs"
