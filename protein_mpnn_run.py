...
def main(args):
    ...
    if args.path_to_model_weights:
        ...
        k = file_path.rfind("/")
        # Look for model weights in original_weights directory
        original_weights_path = file_path[:k] + '/original_weights/'
        if os.path.exists(original_weights_path):
            if args.ca_only:
                print("Using CA-ProteinMPNN!")
                model_folder_path = original_weights_path + 'ca_model_weights/'
                ...
                if args.use_soluble_model:
                    ...
                    model_folder_path = original_weights_path + 'soluble_model_weights/'
                else:
                    model_folder_path = original_weights_path + 'vanilla_model_weights/'
        else:
            # Fallback to original behavior if original_weights doesn't exist
            if args.ca_only:
                print("Using CA-ProteinMPNN!")
