...
def main(args):
    ...
    with ProcessPoolExecutor(max_workers=12) as executor:
        ...
        # CA-only extraction has already happened inside get_pdbs
        dataset_train = StructureDatasetPDB(pdb_dict_train, truncate=None, max_length=args.max_protein_length)
        dataset_valid = StructureDatasetPDB(pdb_dict_valid, truncate=None, max_length=args.max_protein_length)
        ...
        for e in tqdm(range(args.num_epochs), desc='Training Progress', total=args.num_epochs):
            ...
            if e % args.reload_data_every_n_epochs == 0:
                if reload_c != 0:
                    ...
                    # CA-only extraction already done in get_pdbs → rebuild datasets directly
                    dataset_train = StructureDatasetPDB(pdb_dict_train, truncate=None, max_length=args.max_protein_length)
                    dataset_valid = StructureDatasetPDB(pdb_dict_valid, truncate=None, max_length=args.max_protein_length)
