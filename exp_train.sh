#!/usr/bin/env bash
#SBATCH --job-name=exp_train         # changed name
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --gres=gpu:1
#SBATCH --output=logs/exp_%j.out     # logs/ca_JOBID.out
...
JOB_TAG="1_e${EPOCHS}_b${BATCH_SIZE}_n${NEIGHBORS}_noise${NOISE}"

# Define output and log directories
OUTPUT_DIR="outputs/exp${JOB_TAG}"
LOG_DIR="logs"
...
echo "Exp training job completed."